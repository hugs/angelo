...
Angelo.prototype.initPlayer = function() {
  ...
  this.player  = spawn(this.nodeBinary, [player_path, 3000], {detached:false});
...
Angelo.prototype.initActor = function() {
  ...
  this.actor.on('loaded', function(tree) {
    process.stdout.write('Client received message: loaded\n');
    if (tree) {
      process.stdout.write('Client received load: ' + JSON.stringify(tree,null,2) + '\n');
    }
  });
  ...
  this.actor.on('stopped', function() {
